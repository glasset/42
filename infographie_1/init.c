...
/*   Updated: 2014/02/12 14:40:14 by glasset          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
#include <stdlib.h>
#include <fcntl.h>
...
void		print_px(t_ray *ray, void *img, t_vec *index)
{
	...
	if (spheres.z == -1.0 && plans.x == -1.0)
		...
		put_px_to_img(img, index->x, index->y, spheres.y * 100 * a, spheres.y * 100, spheres.y * 150);
	else
		put_px_to_img(img, index->x, index->y, plans.y *150 * plans.x, plans.y * 50, plans.y * 100);
}
