...
static int		sub_check_diagonal(char p, char **board, int line, int col)
{
	int		dline;
	int		dcol;
	int		i;
	...
	dline = line;
	dcol = col;
	while (++dline < MIN_L && ++dcol <= MIN_C && board[dline][dcol] == p)
		i++;
	dline = line;
	dcol = col;
	while (--dline > 0 && --dcol >= 0 && board[dline][dcol] == p)
		i++;
	if (i >= 4)
		return (1);
	return (0);
...
static int		check_diagonal(char p, char **board, int line, int col)
{
	int		dline;
	int		dcol;
	int		i;

	if (sub_check_diagonal(p, board, line, col))
		return (1);
	i = 1;
	dline = line;
	dcol = col;
	while (++dline < MIN_L && --dcol >= 0 && board[dline][dcol] == p)
		i++;
	dline = line;
	dcol = col;
	while (--dline >= 0 && ++dcol <= MIN_C && board[dline][dcol] == p)
		i++;
	if (i >= 4)
		return (1);
	return (0);
...
int				check_victory(char p, t_env *e, int curr_col)
{
	...
	if (check_horizontal(p, e->board[e->curr_line], curr_col - 1))
		return(1);
	if (check_diagonal(p, e->board, e->curr_line, curr_col - 1))
		return(1);
	return (0);
}
