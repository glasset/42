...
class DefaultController extends Controller
{
    public function indexAction()
	{
		...
		$res = $rep->findby(array('dates' => new \DateTime(date('Y-m-d'))));
        return $this->render('RecowinCalendarBundle:Default:index.html.twig', array('today' => $res));
	}
	...
	public function eventAction($years, $month, $day){
		...
		return $this->render('RecowinCalendarBundle:Default:event.html.twig', array('res' => $res, 'm' => $month, 'y' => $years, 'ds' => $day));
	}
	...
	public function delAction($id, $years, $month, $day){
	$em = $this->getDoctrine()->getManager();
    $eve = $em->getRepository('RecowinCalendarBundle:Eventc')->find($id);
	$em->remove($eve);
	$em->flush();
	$rep = $this->getDoctrine()
				->getRepository('RecowinCalendarBundle:Eventc');
